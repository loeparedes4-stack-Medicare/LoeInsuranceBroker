...
export async function renderCard(settings:any,name:string){
 ready ??= Deno.readFile(new URL('./assets/index_bg.wasm',import.meta.url)).then(bytes=>initWasm(bytes));await ready;
 if(!settings.photo_path || !settings.logo_path)throw new Error('Configura la foto del propietario y el logo');
 const [photo,logo,font]=await Promise.all([brand(settings.photo_path),brand(settings.logo_path),Deno.readFile(new URL('./assets/Inter.ttf',import.meta.url))]);
